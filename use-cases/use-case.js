(function () {
  ...
  var CASES = {
    "combine-web-pages-into-one-html": {
      ...
      cta: {
        en: { title: "Turn related pages into one HTML file.", text: "Page collection and HTML generation stay on your device. Free keeps all selected pages in one collection, pauses every 10 pages for confirmation, and saves one final HTML when complete. Pro skips these 10-page confirmations and continues automatically up to your chosen safety limit." },
        ja: { title: "関連ページを、1つのHTMLにまとめよう。", text: "ページ収集とHTML生成は端末内で処理します。無料プランは選択した全ページを同じ収集に保持し、10ページごとに続行を確認して、完了時に1つのHTMLを保存します。Proはこの途中確認を省略し、指定した安全上限まで自動で続行します。" },
        es: { title: "Reúne páginas relacionadas en un único archivo HTML.", text: "La recopilación de páginas y la generación del HTML se realizan en tu dispositivo. Free mantiene todas las páginas seleccionadas en una sola recopilación, pide confirmación cada 10 páginas y guarda un HTML final al terminar. Pro omite estas confirmaciones y continúa automáticamente hasta el límite de seguridad elegido." },
        fr: { title: "Regroupez les pages associées dans un seul fichier HTML.", text: "La collecte des pages et la génération du HTML s’effectuent sur votre appareil. Free conserve toutes les pages sélectionnées dans une seule collecte, demande confirmation toutes les 10 pages et enregistre un HTML final à la fin. Pro omet ces confirmations et poursuit automatiquement jusqu’à la limite de sécurité choisie." },
        de: { title: "Fassen Sie zugehörige Seiten in einer HTML-Datei zusammen.", text: "Die Seitensammlung und HTML-Erstellung erfolgen auf Ihrem Gerät. Free behält alle gewählten Seiten in einer Sammlung, fragt nach jeweils 10 Seiten, ob fortgefahren werden soll, und speichert am Ende eine HTML-Datei. Pro überspringt diese Bestätigungen und fährt bis zum gewählten Sicherheitslimit automatisch fort." },
        it: { title: "Riunisci le pagine correlate in un unico file HTML.", text: "La raccolta delle pagine e la generazione del file HTML avvengono sul dispositivo. Free mantiene tutte le pagine selezionate in un’unica raccolta, chiede conferma ogni 10 pagine e salva un HTML finale al termine. Pro salta queste conferme e continua automaticamente fino al limite di sicurezza scelto." },
        ko: { title: "관련 페이지를 하나의 HTML 파일로 합치세요.", text: "페이지 수집 및 HTML 생성은 기기에서 실행됩니다. Free는 선택한 모든 페이지를 하나의 수집에 유지하고 10페이지마다 계속할지 확인한 뒤 완료 시 HTML 하나를 저장합니다. Pro는 이 확인을 건너뛰고 선택한 안전 한도까지 자동으로 계속합니다." },
        pt_BR: { title: "Reúna páginas relacionadas em um único arquivo HTML.", text: "A coleta de páginas e a geração do HTML são executadas no dispositivo. O Free mantém todas as páginas selecionadas em uma única coleta, pede confirmação a cada 10 páginas e salva um HTML final ao terminar. O Pro ignora essas confirmações e continua automaticamente até o limite de segurança escolhido." },
        zh_CN: { title: "将相关页面合并为一个 HTML 文件。", text: "页面收集和 HTML 生成均在您的设备上运行。Free 将所有选定页面保留在同一次收集中，每 10 页确认是否继续，完成后保存一个最终 HTML。Pro 跳过这些确认，并自动继续到您设定的安全上限。" },
        zh_TW: { title: "將相關頁面合併成一個 HTML 檔案。", text: "頁面收集和 HTML 產生均在您的裝置上執行。Free 將所有選定頁面保留在同一次收集中，每 10 頁確認是否繼續，完成後儲存一個最終 HTML。Pro 略過這些確認，並自動繼續到您設定的安全上限。" }
      },
      copy: {
        en: c(
          ...
          [
            ...
            { q: "Is page content uploaded to your servers?", a: "No. Page collection and HTML generation run on your device. Free keeps all selected pages in one collection, pauses every 10 pages for confirmation, and saves one final HTML when complete. Pro skips these 10-page confirmations and continues automatically up to the chosen safety limit." },
        ...
        ja: c(
          ...
          [
            ...
            { q: "ページ内容はサーバーへアップロードされますか？", a: "いいえ。ページ収集とHTML生成は端末内で処理します。無料プランは選択した全ページを同じ収集に保持し、10ページごとに続行を確認して、完了時に1つのHTMLを保存します。Proはこの途中確認を省略し、指定した安全上限まで自動で続行します。" },
        ...
        de: c(
          ...
          [{ q: "Kann ich auswählen, welche Seiten aufgenommen werden?", a: "Ja. Prüfen Sie vor der Sammlung die Seitentitel, entfernen Sie nicht benötigte Seiten und sortieren Sie die übrigen neu." }, { q: "Kann ich das Ergebnis offline lesen?", a: "Ja. Betten Sie Bilder für die Offline-Nutzung ein oder behalten Sie Online-Verweise bei, um die HTML-Datei kleiner zu halten. Verlinkte Dokumente können ebenfalls zusammen mit ihr gespeichert werden." }, { q: "Werden Seiteninhalte auf Ihre Server hochgeladen?", a: "Nein. Alles wird auf Ihrem Gerät verarbeitet. Free behält die Seiten in einer Sammlung, fragt nach jeweils 10 Seiten, ob fortgefahren werden soll, und speichert am Ende eine HTML-Datei. Pro überspringt diese Bestätigungen und fährt bis zum gewählten Sicherheitslimit fort." }, { q: "Worin unterscheidet sich dies von einem Tool zum Speichern einzelner Seiten wie SingleFile?", a: "Tools zum Speichern einzelner Seiten sichern jeweils die aktuell geöffnete Seite. Grab All Files kann zugehörige Seiten finden, Ihnen das Prüfen und Neuanordnen ermöglichen und die ausgewählten lesbaren Inhalte in einer HTML-Datei mit gemeinsamem Inhaltsverzeichnis zusammenfassen." }]
        ),
        it: c(
          ...
          [{ q: "Posso scegliere quali pagine includere?", a: "Sì. Controlla i titoli prima della raccolta, rimuovi le pagine che non ti servono e riordina le altre." }, { q: "Posso leggere il risultato offline?", a: "Sì. Incorpora le immagini per la lettura offline oppure mantieni i riferimenti online per ridurre le dimensioni del file HTML. Anche i documenti collegati possono essere salvati insieme al file." }, { q: "Il contenuto delle pagine viene caricato sui vostri server?", a: "No. Tutto viene elaborato sul dispositivo. Free mantiene le pagine in un’unica raccolta, chiede conferma ogni 10 pagine e salva un HTML finale. Pro salta queste conferme e continua fino al limite di sicurezza scelto." }, { q: "In cosa si differenzia da uno strumento come SingleFile, che salva una sola pagina?", a: "Gli strumenti di questo tipo conservano la pagina attualmente aperta. Grab All Files può trovare le pagine correlate, consentirti di controllarle e riordinarle, quindi riunire il contenuto leggibile selezionato in un unico file HTML con un indice comune." }]
        ),
        ko: c(
          ...
          [{ q: "포함할 페이지를 직접 선택할 수 있나요?", a: "네. 수집 전에 페이지 제목을 검토하고 필요 없는 페이지를 제거한 뒤 나머지 페이지의 순서를 바꿀 수 있습니다." }, { q: "결과를 오프라인에서 읽을 수 있나요?", a: "네. 이미지를 포함해 오프라인에서 읽거나 온라인 참조를 유지해 HTML 파일 크기를 줄일 수 있습니다. 링크된 문서도 HTML 파일과 함께 저장할 수 있습니다." }, { q: "페이지 콘텐츠가 서버에 업로드되나요?", a: "아니요. 모든 처리는 기기에서 실행됩니다. Free는 페이지를 하나의 수집에 유지하고 10페이지마다 확인한 뒤 HTML 하나를 저장합니다. Pro는 이 확인을 건너뛰고 선택한 안전 한도까지 계속합니다." }, { q: "SingleFile 같은 단일 페이지 저장 도구와는 무엇이 다른가요?", a: "단일 페이지 저장 도구는 현재 열려 있는 페이지만 보존합니다. Grab All Files는 관련 페이지를 탐색하고, 사용자가 이를 검토하고 순서를 바꿀 수 있게 하며, 선택한 읽기 쉬운 콘텐츠를 통합 목차가 있는 하나의 HTML 파일로 합칩니다." }]
        ),
        pt_BR: c(
        ...
        zh_CN: c(
          ...
          [{ q: "可以选择要收录哪些页面吗？", a: "可以。收集前请检查页面标题，删除不需要的页面，并调整其余页面的顺序。" }, { q: "结果可以离线阅读吗？", a: "可以。您可以嵌入图片以便离线阅读，也可以保留在线引用以减小 HTML 文件的大小。链接的文档也可与 HTML 文件一并保存。" }, { q: "页面内容会上传到你们的服务器吗？", a: "不会。所有处理均在您的设备上完成。Free 将页面保留在同一次收集中，每 10 页确认一次并保存一个最终 HTML。Pro 跳过这些确认并继续到所选的安全上限。" }, { q: "这与 SingleFile 等单页面保存工具有何不同？", a: "单页面保存工具只会保留当前打开的页面。Grab All Files 可以查找相关页面，让您检查并调整其顺序，再将所选页面中便于阅读的内容合并为一个带统一目录的 HTML 文件。" }]
        ),
        zh_TW: c(
          ...
          [{ q: "可以選擇要收錄哪些頁面嗎？", a: "可以。收集前請檢視頁面標題，移除不需要的頁面，並調整其餘頁面的順序。" }, { q: "結果可以離線閱讀嗎？", a: "可以。您可以嵌入圖片以供離線閱讀，也可以保留線上參照以縮小 HTML 檔案。連結的文件也可與 HTML 檔案一併儲存。" }, { q: "頁面內容會上傳到你們的伺服器嗎？", a: "不會。所有處理均在您的裝置上完成。Free 將頁面保留在同一次收集中，每 10 頁確認一次並儲存一個最終 HTML。Pro 略過這些確認並繼續到所選的安全上限。" }, { q: "這與 SingleFile 等單頁儲存工具有何不同？", a: "單頁儲存工具只會保留目前開啟的頁面。Grab All Files 可以尋找相關頁面，讓您檢視並調整順序，再將所選頁面中便於閱讀的內容合併成一個附有統一目錄的 HTML 檔案。" }]
        )
      }
    },
    "download-all-pdfs": {

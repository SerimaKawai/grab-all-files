(function () {
  ...
  var CASES = {
    "combine-web-pages-into-one-html": {
      ...
      cta: {
        en: { title: "Turn related pages into one HTML file.", text: "Page collection and HTML generation stay on your device. Free collects up to 10 pages per run, saved as one HTML file; run it again for the rest. Pro removes the per-run limit and continues automatically up to your chosen safety limit." },
        ja: { title: "関連ページを、1つのHTMLにまとめよう。", text: "ページ収集とHTML生成は端末内で処理します。無料プランは1回に10ページずつ収集し、1つのHTMLとして保存します（繰り返せば続きも収集できます）。Proは1回あたりの上限がなく、指定した安全上限まで自動で続行します。" },
        es: { title: "Reúne páginas relacionadas en un único archivo HTML.", text: "La recopilación de páginas y la generación del HTML se realizan en tu dispositivo. Free recopila hasta 10 páginas por ejecución y las guarda como un HTML; repite la ejecución para el resto. Pro elimina ese límite por ejecución y continúa automáticamente hasta el límite de seguridad que elijas." },
        fr: { title: "Regroupez les pages associées dans un seul fichier HTML.", text: "La collecte des pages et la génération du fichier HTML s’effectuent sur votre appareil. Free collecte jusqu’à 10 pages par exécution, enregistrées dans un seul HTML ; relancez-la pour le reste. Pro supprime cette limite par exécution et poursuit automatiquement jusqu’à la limite de sécurité choisie." },
        de: { title: "Fassen Sie zugehörige Seiten in einer HTML-Datei zusammen.", text: "Die Seitensammlung und HTML-Erstellung erfolgen auf Ihrem Gerät. Free sammelt bis zu 10 Seiten pro Durchlauf und speichert sie als eine HTML-Datei; für den Rest starten Sie einfach erneut. Pro hebt dieses Limit pro Durchlauf auf und fährt bis zum gewählten Sicherheitslimit automatisch fort." },
        it: { title: "Riunisci le pagine correlate in un unico file HTML.", text: "La raccolta delle pagine e la generazione del file HTML avvengono sul tuo dispositivo. Free raccoglie fino a 10 pagine per esecuzione, salvate in un unico HTML; ripeti l’esecuzione per il resto. Pro elimina questo limite per esecuzione e continua automaticamente fino al limite di sicurezza scelto." },
        ko: { title: "관련 페이지를 하나의 HTML 파일로 합치세요.", text: "페이지 수집 및 HTML 생성은 기기에서 실행됩니다. Free는 실행당 최대 10페이지를 하나의 HTML로 저장하며, 반복 실행하면 나머지도 수집할 수 있습니다. Pro는 실행당 제한이 없으며 선택한 안전 한도까지 자동으로 계속합니다." },
        pt_BR: { title: "Reúna páginas relacionadas em um único arquivo HTML.", text: "A coleta de páginas e a geração do HTML são executadas no seu dispositivo. No Free, a coleta salva até 10 páginas por execução como um HTML; repita a execução para o restante. No Pro, não há limite por execução e ela continua automaticamente até o limite de segurança escolhido." },
        zh_CN: { title: "将相关页面合并为一个 HTML 文件。", text: "页面收集和 HTML 生成均在您的设备上运行。Free 每次运行最多收集 10 个页面并保存为一个 HTML，重复运行即可收集其余页面；Pro 没有每次运行的数量限制，会自动继续，直到您设定的安全上限。" },
        zh_TW: { title: "將相關頁面合併成一個 HTML 檔案。", text: "頁面收集和 HTML 產生均在您的裝置上執行。Free 每次執行最多收集 10 個頁面並儲存為一個 HTML，重複執行即可收集其餘頁面；Pro 沒有每次執行的數量限制，會自動繼續，直到您設定的安全上限。" }
      },
      copy: {
        en: c(
          ...
          [
            ...
            { q: "Is page content uploaded to your servers?", a: "No. Page collection and HTML generation run on your device. The Free plan collects up to 10 pages per run, saved as one HTML file; run it again for the rest. Pro removes the per-run limit and continues automatically up to the chosen safety limit." },
        ...
        ja: c(
          ...
          [
            ...
            { q: "ページ内容はサーバーへアップロードされますか？", a: "いいえ。ページ収集とHTML生成は端末内で処理します。無料プランは1回に10ページずつ収集し、1つのHTMLとして保存します（繰り返せば続きも収集できます）。Proは1回あたりの上限がなく、指定した安全上限まで自動で続行します。" },
        ...
        de: c(
          ...
          [{ q: "Kann ich auswählen, welche Seiten aufgenommen werden?", a: "Ja. Prüfen Sie vor der Sammlung die Seitentitel, entfernen Sie nicht benötigte Seiten und sortieren Sie die übrigen neu." }, { q: "Kann ich das Ergebnis offline lesen?", a: "Ja. Betten Sie Bilder für die Offline-Nutzung ein oder behalten Sie Online-Verweise bei, um die HTML-Datei kleiner zu halten. Verlinkte Dokumente können ebenfalls zusammen mit ihr gespeichert werden." }, { q: "Werden Seiteninhalte auf Ihre Server hochgeladen?", a: "Nein. Die Seitensammlung und HTML-Erstellung erfolgen auf Ihrem Gerät. Free sammelt bis zu 10 Seiten pro Durchlauf und speichert sie als eine HTML-Datei; für den Rest starten Sie einfach erneut. Pro hebt dieses Limit pro Durchlauf auf und fährt bis zum gewählten Sicherheitslimit automatisch fort." }, { q: "Worin unterscheidet sich dies von einem Tool zum Speichern einzelner Seiten wie SingleFile?", a: "Tools zum Speichern einzelner Seiten sichern jeweils die aktuell geöffnete Seite. Grab All Files kann zugehörige Seiten finden, Ihnen das Prüfen und Neuanordnen ermöglichen und die ausgewählten lesbaren Inhalte in einer HTML-Datei mit gemeinsamem Inhaltsverzeichnis zusammenfassen." }]
        ),
        it: c(
          ...
          [{ q: "Posso scegliere quali pagine includere?", a: "Sì. Controlla i titoli prima della raccolta, rimuovi le pagine che non ti servono e riordina le altre." }, { q: "Posso leggere il risultato offline?", a: "Sì. Incorpora le immagini per la lettura offline oppure mantieni i riferimenti online per ridurre le dimensioni del file HTML. Anche i documenti collegati possono essere salvati insieme al file." }, { q: "Il contenuto delle pagine viene caricato sui vostri server?", a: "No. La raccolta delle pagine e la generazione del file HTML avvengono sul tuo dispositivo. Free raccoglie fino a 10 pagine per esecuzione, salvate in un unico HTML; ripeti l’esecuzione per il resto. Pro elimina questo limite per esecuzione e continua automaticamente fino al limite di sicurezza scelto." }, { q: "In cosa si differenzia da uno strumento come SingleFile, che salva una sola pagina?", a: "Gli strumenti di questo tipo conservano la pagina attualmente aperta. Grab All Files può trovare le pagine correlate, consentirti di controllarle e riordinarle, quindi riunire il contenuto leggibile selezionato in un unico file HTML con un indice comune." }]
        ),
        ko: c(
        ...
        zh_CN: c(
          ...
          [{ q: "可以选择要收录哪些页面吗？", a: "可以。收集前请检查页面标题，删除不需要的页面，并调整其余页面的顺序。" }, { q: "结果可以离线阅读吗？", a: "可以。您可以嵌入图片以便离线阅读，也可以保留在线引用以减小 HTML 文件的大小。链接的文档也可与 HTML 文件一并保存。" }, { q: "页面内容会上传到你们的服务器吗？", a: "不会。页面收集和 HTML 生成均在您的设备上运行。Free 每次运行最多收集 10 个页面并保存为一个 HTML，重复运行即可收集其余页面；Pro 没有每次运行的数量限制，会自动继续，直到所选的安全上限。" }, { q: "这与 SingleFile 等单页面保存工具有何不同？", a: "单页面保存工具只会保留当前打开的页面。Grab All Files 可以查找相关页面，让您检查并调整其顺序，再将所选页面中便于阅读的内容合并为一个带统一目录的 HTML 文件。" }]
        ),
        zh_TW: c(
          ...
          [{ q: "可以選擇要收錄哪些頁面嗎？", a: "可以。收集前請檢視頁面標題，移除不需要的頁面，並調整其餘頁面的順序。" }, { q: "結果可以離線閱讀嗎？", a: "可以。您可以嵌入圖片以供離線閱讀，也可以保留線上參照以縮小 HTML 檔案。連結的文件也可與 HTML 檔案一併儲存。" }, { q: "頁面內容會上傳到你們的伺服器嗎？", a: "不會。頁面收集和 HTML 產生均在您的裝置上執行。Free 每次執行最多收集 10 個頁面並儲存為一個 HTML，重複執行即可收集其餘頁面；Pro 沒有每次執行的數量限制，會自動繼續，直到所選的安全上限。" }, { q: "這與 SingleFile 等單頁儲存工具有何不同？", a: "單頁儲存工具只會保留目前開啟的頁面。Grab All Files 可以尋找相關頁面，讓您檢視並調整順序，再將所選頁面中便於閱讀的內容合併成一個附有統一目錄的 HTML 檔案。" }]
        )
      }
    },
    "download-all-pdfs": {

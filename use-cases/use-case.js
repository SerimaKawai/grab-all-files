(function () {
  ...
  var UI = {
    en: {
      ...
      trial: "Free — 20 files per run + 1 chosen page as HTML",
      ...
      ctaText: "Free saves up to 20 files per run and exactly one chosen related page as HTML. Images and directly linked files on that page have no plan-based count cap. Pro ($19.99, one-time) removes the file cap and combines multiple selected page bodies into one HTML.",
    ...
    ja: {
      ...
      trial: "無料版 — 1回20ファイル＋選んだ1ページをHTML保存",
      local: "PDF結合・ZIP化・CSV出力はブラウザ内でローカル処理",
      privacy: "ファイルは元サイトから端末へ直接ダウンロード",
      ctaTitle: "実際のページで Grab All Files を試せます。",
      ctaText: "無料版はファイルを1回20件まで保存し、関連候補から選んだ1ページをHTML保存します。そのページ内の画像・直接リンクされたファイルにはプラン上の件数制限がありません。Pro（$19.99・買い切り）はファイル上限を解除し、複数ページ本文を1つのHTMLに結合します。",
      skip: "本文へスキップ",
    ...
    es: {
      ...
      trial: "Free — 20 archivos por ejecución + 1 página elegida en HTML",
    ...
    fr: {
      ...
      trial: "Free — 20 fichiers par lot + 1 page choisie en HTML",
      ...
      ctaText: "Free enregistre jusqu’à 20 fichiers par lot et exactement une page associée choisie en HTML. Les images et fichiers directement liés sur cette page n’ont pas de plafond lié au forfait. Pro (19,99 $, achat unique) supprime la limite et regroupe le contenu de plusieurs pages sélectionnées.",
    ...
    de: {
      ...
      trial: "Free — 20 Dateien je Durchlauf + 1 gewählte Seite als HTML",
      ...
      ctaText: "Free speichert bis zu 20 Dateien pro Durchlauf und genau eine gewählte zugehörige Seite als HTML. Bilder und direkt verlinkte Dateien auf dieser Seite haben kein planbedingtes Mengenlimit. Pro (19,99 $, einmalig) hebt das Dateilimit auf und bündelt mehrere ausgewählte Seiteninhalte.",
    ...
    it: {
      ...
      trial: "Free — 20 file per esecuzione + 1 pagina scelta in HTML",
      ...
      ctaText: "Free salva fino a 20 file per esecuzione ed esattamente una pagina correlata scelta come HTML. Immagini e file collegati direttamente in quella pagina non hanno un limite numerico del piano. Pro ($19.99, una tantum) rimuove il limite e unisce il contenuto di più pagine selezionate.",
    ...
    ko: {
      ...
      trial: "Free — 실행당 파일 20개 + 선택한 1페이지 HTML 저장",
      local: "PDF 병합, ZIP, CSV는 브라우저에서 로컬 처리",
      privacy: "파일은 원본 사이트에서 기기로 직접 다운로드됩니다",
      ctaTitle: "브라우저에서 Grab All Files를 사용해 보세요.",
      ctaText: "Free는 실행당 파일을 최대 20개 저장하고 관련 후보 중 정확히 1페이지를 HTML로 저장합니다. 그 페이지의 이미지와 직접 연결된 파일에는 플랜상 개수 제한이 없습니다. Pro($19.99, 1회 결제)는 파일 제한을 없애고 선택한 여러 페이지 본문을 하나의 HTML로 결합합니다.",
      skip: "본문으로 건너뛰기",
      primaryNav: "주요 탐색",
      themeToggle: "테마 전환",
      languageLabel: "언어",
    ...
    pt_BR: {
      ...
      trial: "Free — 20 arquivos por execução + 1 página escolhida em HTML",
      ...
      ctaText: "O Free salva até 20 arquivos por execução e exatamente uma página relacionada escolhida como HTML. Imagens e arquivos diretamente vinculados nessa página não têm limite de quantidade do plano. O Pro ($19.99, compra única) remove o limite e reúne o conteúdo de várias páginas selecionadas.",
    ...
    zh_CN: {
      ...
      trial: "Free — 每次20个文件＋将所选1个页面保存为HTML",
      local: "PDF合并、ZIP和CSV均在浏览器本地处理",
      privacy: "文件从源网站直接下载到您的设备",
      ctaTitle: "在浏览器中试用 Grab All Files。",
      ctaText: "Free 每次最多保存20个文件，并从相关候选中仅选择1个页面保存为HTML。该页面内的图片和直接链接文件没有套餐数量上限。Pro（$19.99，一次性购买）解除文件上限，并将多个所选页面正文合并为一个HTML。",
      skip: "跳至主要内容",
      primaryNav: "主导航",
      themeToggle: "切换主题",
      languageLabel: "语言",
    ...
    zh_TW: {
      ...
      trial: "Free — 每次20個檔案＋將所選1個頁面儲存為HTML",
      local: "PDF合併、ZIP和CSV均在瀏覽器本機處理",
      privacy: "檔案從來源網站直接下載到您的裝置",
      ctaTitle: "在瀏覽器中試用 Grab All Files。",
      ctaText: "Free 每次最多儲存20個檔案，並從相關候選中只選1個頁面儲存為HTML。該頁面內的圖片和直接連結檔案沒有方案數量上限。Pro（$19.99，一次購買）解除檔案上限，並將多個所選頁面正文合併為一個HTML。",
      skip: "跳至主要內容",
      primaryNav: "主要導覽",
      themeToggle: "切換主題",
      languageLabel: "語言",
    ...
    "download-all-pdfs": {
      ...
      copy: {
        en: c(
          ...
          "Download linked PDFs from a page or its folder on Chrome, Edge or Firefox. Filter results, save up to 20 files per run for free, or merge PDFs locally.",
          "PDF collection",
        ...
        ja: c(
          ...
          [{ q: "PDFビューアやiframe内のPDFも見つかりますか？", a: "はい。iframe、ビューアURL、fetch/XHRレスポンス、blob URL、CMSルートなど直接リンクが見えにくい場所も確認します。" }, { q: "複数ページをクロールできますか？", a: "はい。深さを上げると、起点ページのフォルダ配下にあるリンク先を巡回します。例えば /~name/course.html が起点なら /~name/ 配下が対象で、大学のサイト全体には広がりません。" }, { q: "PDF結合でファイルはアップロードされますか？", a: "いいえ。PDF結合はブラウザ内でローカル処理されます。" }, { q: "PDFを無料で一括ダウンロードできますか？", a: "はい。無料版は1回20件まで選択して保存できます。Proではこの件数上限がなくなります。ZIP保存、ローカルPDF結合、ファイル情報CSV出力も無料版で利用できます。" }, { q: "サイト内のすべてのPDFを必ず保存できますか？", a: "保証はできません。見つかったリンク、巡回の深さ、サイトへのアクセス権限、ブラウザーの対応状況によって結果が変わります。失敗したダウンロードは再試行するか、原因付きの失敗URL一覧をCSV出力して確認できます。ログインやサイトの制限を回避する機能ではありません。" }]
        ),
        es: c(

...
- Chrome package: C:\xampp\htdocs\filescanner\dist\grab-all-files-chrome-v5.1.5.zip
- Edge package: C:\xampp\htdocs\filescanner\dist\grab-all-files-edge-v5.1.5.zip
- Firefox package: C:\xampp\htdocs\filescanner\dist\grab-all-files-firefox-v5.1.5.zip
...
This extension is free to use. A single download saves up to 20 files at once; repeat the download to save the rest, still free. A one-time Pro license removes this per-download limit so everything saves at once and activates on up to 3 devices.
...
Key features:
- Scan the current page, paste one or many URLs, or crawl a whole site (depth 0–5 or unlimited, up to 100,000 pages)
...
- 38 file-type categories (documents, images, spreadsheets, e-books, CAD/BIM, GIS, camera RAW, AI/ML models, and more) plus your own custom extensions
...
About permissions:
Grab All Files requests broad HTTP/HTTPS host permissions so the user can scan any site of their choice — the target domain cannot be known in advance. The extension does not send scan history, page HTML, browsing activity, or file contents to any developer-controlled server. Outbound traffic to a developer server is limited to HTTPS calls to the Supabase license backend for license verification, plus a small set of plan-interaction events (when a free-plan limit or upgrade prompt is shown, and whether you choose to upgrade or stay on the free plan) that contain only a randomly generated device ID, your plan state, and a feature label — never page content, URLs, file names, or personal data.
```
...
本拡張機能は無料で利用できます。1 回のダウンロードで最大 20 ファイルを保存でき、残りは同じダウンロードを繰り返せば無料で保存できます。買い切りの Pro ライセンスを購入すると、このダウンロードあたりの上限が解除されて一度にすべて保存でき、最大 3 台の端末で有効化できます。

有料ライセンスは Microsoft や Google ではなく、Tetsunobu Kawai が販売します。決済は拡張機能内ではなく、外部購入ページの Stripe Checkout で処理されます。

主な機能:
- 現在のページのスキャン、URL の貼り付け（複数 URL の一括も可）、サイト全体のクロール（深度 0〜5 または無制限、最大 100,000 ページ）
...
- 38 のファイルカテゴリ（文書・画像・表計算・電子書籍・CAD/BIM・GIS・カメラ RAW・AI/ML モデルなど）＋ユーザー定義のカスタム拡張子
...
権限について:
Grab All Files は、ユーザーが任意のサイトをスキャンできるよう、広い HTTP/HTTPS ホスト権限を要求します。対象ドメインは事前に分かりません。本拡張機能は、スキャン履歴・ページ HTML・閲覧行動・ファイル内容を開発者管理のサーバーには送信しません。開発者サーバーへの外向き通信は、ライセンス確認のための Supabase ライセンスバックエンドへの HTTPS 呼び出しと、ごく少数のプラン操作イベント（無料プランの上限やアップグレード案内が表示されたとき、およびアップグレードするか無料プランのまま続けるかの選択）に限られます。これらのイベントには、ランダムに生成されたデバイス ID、プランの状態、機能ラベルのみが含まれ、ページ内容・URL・ファイル名・個人情報は一切含まれません。
```
...
Esta extensión es gratuita. Una sola descarga guarda hasta 20 archivos a la vez; repite la descarga para guardar el resto, siempre gratis. Una licencia Pro de pago único elimina este límite por descarga para guardarlo todo de una vez y se activa en hasta 3 dispositivos.
...
- Escanea la página actual, pega una o varias URL, o rastrea un sitio entero (profundidad 0–5 o ilimitada, hasta 100.000 páginas)
...
- 38 categorías de tipos de archivo (documentos, imágenes, hojas de cálculo, libros electrónicos, CAD/BIM, GIS, RAW de cámara, modelos de IA/ML y más) más tus propias extensiones personalizadas
...
Sobre los permisos:
Grab All Files solicita permisos amplios de host HTTP/HTTPS para que el usuario pueda escanear cualquier sitio que elija — el dominio objetivo no puede conocerse de antemano. La extensión no envía el historial de escaneo, HTML de la página, actividad de navegación ni el contenido de los archivos a ningún servidor controlado por el desarrollador. El tráfico saliente a un servidor del desarrollador se limita a llamadas HTTPS al backend de licencias de Supabase para la verificación de la licencia, más un pequeño conjunto de eventos de interacción con el plan (cuando se muestra un límite del plan gratuito o un aviso de mejora, y si eliges mejorar o seguir en el plan gratuito) que contienen únicamente un identificador de dispositivo generado aleatoriamente, el estado de tu plan y una etiqueta de función — nunca el contenido de las páginas, URLs, nombres de archivos ni datos personales.
```
...
Cette extension est gratuite. Un seul téléchargement enregistre jusqu'à 20 fichiers à la fois ; répétez le téléchargement pour enregistrer le reste, toujours gratuitement. Une licence Pro à achat unique supprime cette limite par téléchargement afin de tout enregistrer d'un coup et s'active sur jusqu'à 3 appareils.
...
- Analysez la page courante, collez une ou plusieurs URL, ou explorez un site entier (profondeur 0–5 ou illimitée, jusqu'à 100 000 pages)
...
- 38 catégories de types de fichiers (documents, images, feuilles de calcul, livres numériques, CAO/BIM, SIG, RAW d'appareil photo, modèles IA/ML, et plus) ainsi que vos propres extensions personnalisées
...
Grab All Files demande des autorisations larges pour les hôtes HTTP/HTTPS afin que l'utilisateur puisse analyser le site de son choix — le domaine cible ne peut pas être connu à l'avance. L'extension n'envoie pas l'historique d'analyse, le HTML de la page, l'activité de navigation ni le contenu des fichiers vers un serveur contrôlé par le développeur. Le trafic sortant vers un serveur développeur se limite à des appels HTTPS vers le backend de licence Supabase pour la vérification de la licence, ainsi qu'à un petit ensemble d'événements d'interaction avec la formule (lorsqu'une limite de la formule gratuite ou une invitation à passer à la version supérieure est affichée, et si vous choisissez de passer à la version supérieure ou de rester sur la formule gratuite) qui ne contiennent qu'un identifiant d'appareil généré aléatoirement, l'état de votre formule et un libellé de fonction — jamais le contenu des pages, les URLs, les noms de fichiers ni de données personnelles.
```
...
Diese Erweiterung ist kostenlos nutzbar. Ein einzelner Download speichert bis zu 20 Dateien auf einmal; wiederholen Sie den Download, um den Rest zu speichern — weiterhin kostenlos. Eine einmalig zu erwerbende Pro-Lizenz hebt dieses Limit pro Download auf, sodass alles auf einmal gespeichert wird, und wird auf bis zu 3 Geräten aktiviert.
...
Hauptfunktionen:
- Scannen Sie die aktuelle Seite, fügen Sie eine oder mehrere URLs ein oder crawlen Sie eine ganze Website (Tiefe 0–5 oder unbegrenzt, bis zu 100.000 Seiten)
...
- 38 Dateityp-Kategorien (Dokumente, Bilder, Tabellen, E-Books, CAD/BIM, GIS, Kamera-RAW, KI/ML-Modelle und mehr) plus eigene benutzerdefinierte Erweiterungen
...
Grab All Files fordert breite HTTP/HTTPS-Host-Berechtigungen an, damit Sie jede beliebige Website scannen können — die Zieldomäne kann nicht im Voraus bekannt sein. Die Erweiterung sendet keinen Scan-Verlauf, kein Seiten-HTML, keine Browser-Aktivität und keinen Dateiinhalt an einen vom Entwickler kontrollierten Server. Ausgehender Datenverkehr an einen Entwickler-Server beschränkt sich auf HTTPS-Aufrufe an das Supabase-Lizenz-Backend zur Lizenzprüfung sowie auf eine kleine Anzahl von Plan-Interaktionsereignissen (wenn eine Begrenzung des kostenlosen Plans oder eine Upgrade-Aufforderung angezeigt wird und ob Sie sich für ein Upgrade entscheiden oder beim kostenlosen Plan bleiben), die nur eine zufällig generierte Geräte-ID, Ihren Planstatus und eine Funktionsbezeichnung enthalten — niemals Seiteninhalte, URLs, Dateinamen oder personenbezogene Daten.
```
...
Questa estensione è gratuita. Un singolo download salva fino a 20 file per volta; ripeti il download per salvare il resto, sempre gratis. Una licenza Pro con acquisto una tantum rimuove questo limite per download così salvi tutto in una volta e si attiva su un massimo di 3 dispositivi.
...
- Scansiona la pagina corrente, incolla uno o più URL oppure esegui il crawl di un intero sito (profondità 0–5 o illimitata, fino a 100.000 pagine)
...
- 38 categorie di tipi di file (documenti, immagini, fogli di calcolo, e-book, CAD/BIM, GIS, RAW della fotocamera, modelli IA/ML e altro) più le tue estensioni personalizzate
...
Grab All Files richiede permessi ampi per gli host HTTP/HTTPS, in modo che l'utente possa eseguire la scansione di qualsiasi sito a propria scelta — il dominio di destinazione non può essere noto in anticipo. L'estensione non invia la cronologia delle scansioni, l'HTML della pagina, l'attività di navigazione né il contenuto dei file a server controllati dallo sviluppatore. Il traffico in uscita verso un server dello sviluppatore è limitato alle chiamate HTTPS al backend delle licenze Supabase per la verifica della licenza, oltre a un piccolo insieme di eventi di interazione con il piano (quando viene mostrato un limite del piano gratuito o un invito all'upgrade, e se scegli di passare alla versione superiore o di restare sul piano gratuito) che contengono solo un identificatore del dispositivo generato in modo casuale, lo stato del tuo piano e un'etichetta della funzione — mai il contenuto delle pagine, gli URL, i nomi dei file o dati personali.
```
...
이 확장 프로그램은 무료로 사용할 수 있습니다. 한 번의 다운로드로 최대 20개 파일을 저장할 수 있으며, 다운로드를 반복하면 나머지도 무료로 저장할 수 있습니다. 일회성 구매 Pro 라이선스는 이 다운로드당 제한을 없애 한 번에 모두 저장할 수 있게 하며, 최대 3대의 기기에서 활성화됩니다.

유료 라이선스는 Microsoft나 Google이 아닌 Tetsunobu Kawai가 판매합니다. 결제는 확장 프로그램 내부가 아닌 외부 구매 페이지의 Stripe Checkout에서 처리됩니다.

주요 기능:
- 현재 페이지 스캔, URL 붙여넣기(여러 URL 동시 입력 가능), 또는 사이트 전체 크롤링(깊이 0–5 또는 무제한, 최대 100,000페이지)
...
- 38개 파일 유형 카테고리(문서, 이미지, 스프레드시트, 전자책, CAD/BIM, GIS, 카메라 RAW, AI/ML 모델 등) 및 사용자 정의 확장자
...
권한에 대하여:
Grab All Files는 사용자가 원하는 어떤 사이트든 스캔할 수 있도록 광범위한 HTTP/HTTPS 호스트 권한을 요청합니다 — 대상 도메인은 사전에 알 수 없습니다. 본 확장 프로그램은 스캔 기록, 페이지 HTML, 브라우징 활동, 파일 내용을 개발자가 관리하는 서버로 보내지 않습니다. 개발자 서버로의 외부 통신은 라이선스 검증 목적의 Supabase 라이선스 백엔드 HTTPS 호출과, 소수의 플랜 상호작용 이벤트(무료 플랜의 제한이나 업그레이드 안내가 표시될 때, 그리고 업그레이드를 선택하는지 무료 플랜을 유지하는지)에 한정됩니다. 이러한 이벤트에는 무작위로 생성된 기기 ID, 플랜 상태, 기능 라벨만 포함되며, 페이지 내용·URL·파일 이름·개인정보는 절대 포함되지 않습니다.
```
...
Esta extensão é gratuita. Um único download salva até 20 arquivos de uma vez; repita o download para salvar o restante, sempre grátis. Uma licença Pro de compra única remove esse limite por download para salvar tudo de uma vez e é ativada em até 3 dispositivos.
...
Recursos principais:
- Escaneie a página atual, cole uma ou várias URLs ou rastreie um site inteiro (profundidade 0–5 ou ilimitada, até 100.000 páginas)
...
- 38 categorias de tipos de arquivo (documentos, imagens, planilhas, e-books, CAD/BIM, GIS, RAW de câmera, modelos de IA/ML e mais) além das suas próprias extensões personalizadas
...
Sobre as permissões:
O Grab All Files solicita permissões amplas de host HTTP/HTTPS para que o usuário possa escanear qualquer site de sua escolha — o domínio de destino não pode ser conhecido com antecedência. A extensão não envia histórico de varredura, HTML da página, atividade de navegação ou conteúdo de arquivos a nenhum servidor controlado pelo desenvolvedor. O tráfego de saída para um servidor do desenvolvedor se limita a chamadas HTTPS ao backend de licença do Supabase, exclusivamente para verificação de licença. Além disso, é enviado um pequeno conjunto de eventos anônimos de interação com o plano (apenas um ID de dispositivo aleatório, o estado do plano e um rótulo de recurso) quando um limite do plano gratuito ou aviso de upgrade é exibido; nunca conteúdo de páginas, URLs, nomes de arquivos ou conteúdo de arquivos.
```
...
本扩展程序可免费使用。单次下载最多保存 20 个文件，重复下载即可免费保存其余文件。一次性购买的 Pro 授权可解除这一每次下载的上限，让您一次性保存全部文件，并可在最多 3 台设备上激活。

付费授权由 Tetsunobu Kawai 销售，而非由 Microsoft 或 Google 销售。支付通过外部购买页面上的 Stripe Checkout 处理，而非在扩展程序内部进行。

主要功能：
- 扫描当前页面、粘贴一个或多个 URL，或爬取整个网站（深度 0–5 或无限，最多 100,000 页）
...
- 38 个文件类型类别（文档、图片、电子表格、电子书、CAD/BIM、GIS、相机 RAW、AI/ML 模型等）以及您自定义的扩展名
...
关于权限：
Grab All Files 请求广泛的 HTTP/HTTPS 主机权限，以便用户可以扫描自己选择的任何站点 — 目标域名无法事先得知。本扩展程序不会将扫描历史、页面 HTML、浏览行为或文件内容发送到任何由开发者控制的服务器。发往开发者服务器的出站流量仅限于针对授权验证的 Supabase 授权后端 HTTPS 调用。此外，当显示免费方案上限或升级提示时，会发送少量匿名的套餐交互事件（仅含随机设备ID、套餐状态和功能标签），绝不包含页面内容、URL、文件名或文件内容。
```
...
本擴充功能可免費使用。單次下載最多儲存 20 個檔案，重複下載即可免費儲存其餘檔案。一次性購買的 Pro 授權可解除這項每次下載的上限，讓您一次儲存全部檔案，並可在最多 3 台裝置上啟用。

付費授權由 Tetsunobu Kawai 販售，並非由 Microsoft 或 Google 販售。付款透過外部購買頁面上的 Stripe Checkout 處理，並非在擴充功能內進行。

主要功能：
- 掃描目前頁面、貼上一個或多個 URL，或爬取整個網站（深度 0–5 或無限，最多 100,000 頁）
...
- 38 個檔案類型類別（文件、圖片、試算表、電子書、CAD/BIM、GIS、相機 RAW、AI/ML 模型等）以及您自訂的副檔名
...
關於權限：
Grab All Files 要求廣泛的 HTTP/HTTPS 主機權限，以便使用者可以掃描自選的任何網站 — 目標網域無法預先得知。本擴充功能不會將掃描歷程、網頁 HTML、瀏覽行為或檔案內容傳送至任何開發者管控的伺服器。發往開發者伺服器的出站流量，僅限於針對授權驗證的 Supabase 授權後端 HTTPS 呼叫，以及少量方案互動事件（在顯示免費方案上限或升級提示時，以及您選擇升級或繼續使用免費方案時）。這些事件僅包含隨機產生的裝置 ID、您的方案狀態與功能標籤——絕不包含網頁內容、URL、檔案名稱或個人資料。
```
...
- Confirm the submitted package version is 5.1.5.
...
- **Data-practices / privacy form (new in 4.5.6):** declare the optional, anonymous uninstall survey. On uninstall the extension opens grab-all-files.app/uninstall with non-identifying URL params (extension version, UI language, days installed, plan (free or paid)); if the user chooses to submit, those plus the selected reason and an optional free-text comment are stored on the Supabase backend. No device ID, e-mail, or IP is collected. Disclose under the applicable "user activity / feedback" data category; it is covered in privacy-policy.html §5c. (Besides license verification, the extension build sends only a small set of anonymous plan-interaction events — a random device ID, plan state, and a feature label; the uninstall survey is submitted from the website only on user action.)
- Firefox/AMO may request source code for review. The v5.1.5 source ZIP is available at C:\xampp\htdocs\filescanner\grab-all-files-source-v5.1.5.zip and is verified to reproduce dist/firefox byte-for-byte via npm install + npm run build.
